...
driver = WhatsAPIDriver(client='chrome',profile='./User_Data')
...
while True:
    for contact in driver.get_unread(include_me=True, include_notifications=True):
        for message in contact.messages:
            ...
            ch=0
            if message.type=='chat' and message.content=='#adminclose':
                wd.quit()
                driver.quit()
            elif message.type == 'chat' and "#gfg" in message.content:
               try:
